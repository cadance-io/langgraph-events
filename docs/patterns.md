...
- Only once the budget is spent — or the run's `deadline=` [cuts the backoff short](control-flow.md#retries) — does `HandlerRaised` reach `give_up`, which escalates to `Question.GaveUp` ([`Halted`](concepts.md#system-events) subtype). The catcher counts nothing and schedules nothing.
