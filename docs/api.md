...
| `transform_fields` | Decorator | `@transform_fields(fn)` — class-global rewrite of the stored kwargs for a field the class dropped, merged or retyped. `fn` receives a copy of the kwargs and returns the kwargs the constructor receives. Runs after any rename, on payloads from every era, so it must be idempotent. One per class. Same auto-collection as `@backfill`. See [Dropping, merging or retyping a field](event-migrations.md#dropping-merging-or-retyping-a-field) |
...
| `write_baseline` | Function (`…serde.migrations.detect`) | `write_baseline(graph, path, *, allow_removed=False)` — snapshots topology into a v3 baseline: every identity with its cumulative `fields`, the handler names, and a `retired` list of identities the graph no longer reaches. Never erases an identity. `allow_removed` is deprecated and does nothing. See [When to commit the baseline](event-migrations.md#when-to-commit-the-baseline) |
