...
| `EventGraph.unrevivable_threads()` / `.aunrevivable_threads()` | Method | Mapping of thread id to the qualnames its latest checkpoint can no longer revive, settled history and pending interrupt both. Empty when every thread revives. Reads the store, not the baseline, so it stays accurate after `write_baseline(..., allow_removed=True)`. Degrades only through `NamespaceAwareSerde`. Requires a checkpointer. Cost is O(all checkpoints). See [Retiring an Interrupted subclass](event-migrations.md#retiring-an-interrupted-subclass) |
