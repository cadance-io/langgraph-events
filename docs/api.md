...
| `HandlerRaised` | Event | Emitted when a handler raises a `raises=`-declared exception; carries `handler`, `source_event`, `exception`, `abandoned_for_deadline`. With a `retry=` policy it fires only after the budget is spent — or, with `abandoned_for_deadline=True`, once the next backoff would cross the run's `deadline=` |
