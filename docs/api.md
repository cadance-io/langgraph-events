...
| `EventGraph.abandon()` / `.aabandon()` | Method | Settle a paused thread without answering its interrupt (requires checkpointer); returns `None` — see [Ending a pause without answering it](control-flow.md#ending-a-pause-without-answering-it-abandon) |
...
| `Abandoned` | Event | `Halted` subtype emitted by `EventGraph.abandon()` / `.aabandon()`. `.reason` is the caller-supplied reason; `.discarded` is the type name(s) of the interrupt(s) thrown away. Recorded, not dispatched — `@on(Abandoned)` never fires |
