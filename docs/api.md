...
| `EventGraph.threads_paused_on()` / `.athreads_paused_on()` | Method | Configs for every thread whose latest checkpoint has a pending interrupt, optionally filtered to an `Interrupted` class or subclass. Requires a checkpointer. Reads every checkpoint the checkpointer holds. Cost is O(all checkpoints). See [Ending a pause without answering it](control-flow.md#ending-a-pause-without-answering-it-abandon) |
| `EventGraph.unrevivable_threads()` / `.aunrevivable_threads()` | Method | Mapping of thread id to the qualnames its latest checkpoint can no longer revive, wherever the serde met them: settled history, pending interrupt, completed sibling write, or a field nested in a live event. Empty when every thread revives. Reads the store, not the baseline, so it sees a settled thread the coverage gates cannot. Requires a checkpointer with a `NamespaceAwareSerde`, raises `ValueError` otherwise. Cost is O(all checkpoints). See [Retiring an Interrupted subclass](event-migrations.md#retiring-an-interrupted-subclass) |
| `EventGraph.plan_rewrite()` / `.aplan_rewrite()` | Method | `RewriteReport` of the rewrite each thread needs, after the same verification as `rewrite_store()`. Writes nothing. `drop=` names event classes whose stored instances leave the log. `thread_ids=` limits the walk; `None` reads every checkpoint the checkpointer holds, so on Postgres pass the ids from one `SELECT DISTINCT thread_id` query. Requires a checkpointer with a `NamespaceAwareSerde` and `legacy_write=False`, raises `ValueError` otherwise. See [Rewriting the live set](event-migrations.md#rewriting-the-live-set) |
| `EventGraph.rewrite_store()` / `.arewrite_store()` | Method | Rewrites each thread's latest checkpoint under the live migration table, through the checkpointer's `put()` under the same checkpoint id, and drops the stored instances of each `drop=` class from `events` and `_pending`. Same arguments and report as `plan_rewrite()`. A refused thread is not written; its `reason` says what to do. Run while the graph is idle, never inside a rolling deploy window, after a store-level backup. See [Rewriting the live set](event-migrations.md#rewriting-the-live-set) |
...
| `Abandoned` | Event | `Halted` subtype emitted by `EventGraph.abandon()` or `.aabandon()`. `.reason` is the caller-supplied reason. `.discarded` holds the discarded interrupts' **qualnames** (e.g. `Order.ApprovalRequested`, not the bare `ApprovalRequested`). The qualname stays unambiguous under nesting and stable whether or not the class still imports. Values are deduped and joined with `", "` (`""` if none). Match with `in` or `.split(", ")`, never `==`. Recorded, not dispatched: `@on(Abandoned)` never fires |
...
| Export | Type | Description |
|---|---|---|
| `NamespaceAwareSerde` | Class | `JsonPlusSerializer` subclass keying `Event` identity by `(__module__, __qualname__)`. Drop-in for any checkpointer accepting `serde=`. `EventGraph.from_namespaces` auto-wires one scoped to its namespaces when the checkpointer carries the default serde; pass your own via `MemorySaver(serde=...)` to opt out. Kwargs: `migrations=`, `namespaces=`, `legacy_write=`. `legacy_write=True` is refused at construction with an origin-scoped fill, any transform or any split |
| `migrate_from` | Decorator | `@migrate_from(*old_qualnames, in_module=None, backfill=None, transform=None)` — records historic identities a class held. `backfill={field: default}` scopes fills to payloads written under *this* decorator's qualname (one qualname per decorator) — the fan-in case where collapsed origins each pin their own discriminator. `transform=fn` rewrites the stored kwargs of payloads written under this decorator's qualname only, before the rename (one qualname per decorator). `transform=lambda kw: {}` retires an identity onto an empty tombstone. Auto-collected from the namespaces the serde was built with; stacks; not MRO-inherited |
...
| `transform_fields` | Decorator | `@transform_fields(fn)` — the class stage: rewrites the stored kwargs for a field the class dropped, merged or retyped. `fn` receives a copy of the kwargs and returns the kwargs the constructor receives. Runs after any rename, on payloads from every era, so it must be idempotent. One per class. `migrate_from(transform=fn)` is the origin stage. Same auto-collection as `@backfill`. See [Dropping, merging or retyping a field](event-migrations.md#dropping-merging-or-retyping-a-field) |
| `split_event` | Decorator | `@split_event(select, *, targets=(...))` — applied to the source class, whose identity stays stored. `select` receives a copy of the stored kwargs and returns `None` to keep the source, or `(target_class, kwargs)` to build one of `targets` instead. Runs in the class stage, after any rename and `@transform_fields`, before the fills of the resulting identity. `targets` is validated at serde construction. One per class. Same auto-collection as `@transform_fields`. See [Splitting one stored event into two on a payload value](event-migrations.md#splitting-one-stored-event-into-two-on-a-payload-value) |
| `Migration` | Class | Hand-authored migration group for cross-module / composite cases. `Migration.rename(...)` / `Migration.add_field(...)` / `Migration.transform_fields(...)` / `Migration.split_event(...)` are single-op sugar; pass the live target as `to=<class>` / `target=<class>` / `source=<class>` (refactor-safe) instead of `new_module`/`new_qualname` strings. `add_field` or `transform_fields` keyed on a historic `module`/`qualname` is origin-scoped (applies before the rename) — the escape hatch for a per-origin `default_factory` or transform. `split_event(source=Class, select=fn, targets=(...))` keys on the live source only |
| `assert_all_baselined_cover` | Function | `assert_all_baselined_cover(serde, baseline_path)` — weakest coverage gate: set-membership over `revivable_identities()`; raises `MigrationCoverageError` if an identity is neither live nor rename-covered. Walks the baseline's `events` and `retired` lists both. Scoped to the serde's `namespaces=` walk plus `events=`. See [coverage gates](event-migrations.md#coverage-gates) |
| `assert_all_baselined_resolve` | Function | `assert_all_baselined_resolve(serde, baseline_path)` — rename-aware gate: asserts every baselined identity resolves to a live `Event` class **without constructing** it. Walks `events` and `retired` both. For baselines with `__post_init__`-validated, framework, or module-level events |
| `assert_all_baselined_revive` | Function | `assert_all_baselined_revive(serde, baseline_path)` — strongest gate: pushes a synthesized payload for every baselined identity through the real read path and asserts it revives. Walks `events` and `retired` both. A v3 baseline's recorded fields are sent too, so a field the live class dropped fails here. No per-event list to maintain |
...
| `MigrationCoverageError` | Exception | Raised by the event gates (`assert_all_baselined_cover`/`_resolve`/`_revive`); `CoverageError` subclass. `.uncovered` is the offending `(module, qualname)` tuple. `.retired` is the subset the baseline lists as retired |
...
| `write_baseline` | Function (`…serde.migrations.detect`) | `write_baseline(graph, path)` — snapshots topology into a v3 baseline: every identity with its cumulative `fields`, the handler names, and a `retired` list of identities the graph no longer reaches. Never erases an identity. The `allow_removed=` keyword is deprecated and does nothing. See [When to commit the baseline](event-migrations.md#when-to-commit-the-baseline) |
...
| `BaselineRegressionError` | Exception (`…serde.migrations.detect`) | Retained for one release; `write_baseline` no longer raises it. `ValueError` subclass. `.removed` is the tuple of dropped identities |

Raw `RenameEvent` / `AddField` / `TransformFields` / `SplitEvent` operation constructors are the composite escape hatch in `langgraph_events.serde.migrations` — intentionally **not** re-exported at the top level (the decorator + `Migration` sugar is the public tier). `TransformFields(module, qualname, transform)` rewrites the stored kwargs of one identity; the identity picks the stage, the same rule as `AddField`. `SplitEvent(module, qualname, select, targets)` keys on a live identity and builds one of `targets` when `select` returns it; the class stage only. Wrap either like every raw op: `Migration(operations=(TransformFields(module, qualname, transform),))`.
