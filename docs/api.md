...
| `Migration` | Class | Hand-authored migration group for cross-module / composite cases. `Migration.rename(...)` / `Migration.add_field(...)` / `Migration.transform_fields(...)` are single-op sugar; pass the live target as `to=<class>` / `target=<class>` (refactor-safe) instead of `new_module`/`new_qualname` strings. `add_field` or `transform_fields` keyed on a historic `module`/`qualname` is origin-scoped (applies before the rename) — the escape hatch for a per-origin `default_factory` or transform |
...
Raw `RenameEvent` / `AddField` / `TransformFields` operation constructors are the composite escape hatch in `langgraph_events.serde.migrations` — intentionally **not** re-exported at the top level (the decorator + `Migration` sugar is the public tier). `TransformFields(module, qualname, transform)` rewrites the stored kwargs of one identity; the identity picks the stage, the same rule as `AddField`. Wrap it like every raw op: `Migration(operations=(TransformFields(module, qualname, transform),))`.
