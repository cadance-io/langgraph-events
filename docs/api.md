...
- `node_name=` — pin a **stable node identity** (defaults to the function name) so renaming the function never breaks an interrupted checkpoint. `previously=` (str or tuple) — historic node names to keep resumable after a rename. These, and `retry=`, are reserved (a field named `node_name`/`previously`/`retry` can't be matched via `**field_matchers`). Inline `Command.handle()` handlers default their node identity to the **command's `__qualname__`** (e.g. `Order.Place`), not the method name, so it is stable regardless of `handlers=[...]` order; a renamed command declares its historic node names as a class attribute, `previously: ClassVar = (...)` — mirroring `raises`/`invariants` in spelling, but unlike them deliberately **not** MRO-inherited (a historic node name identifies exactly one class). See [Handler renames](event-migrations.md#handler-renames).
...
| `HandlerRaised` | Event | Emitted when a handler raises a `raises=`-declared exception; carries `handler`, `source_event`, `exception`. With a `retry=` policy it fires only after the budget is spent |
| `HandlerRetried` | Event | Emitted before each `retry=` backoff wait; carries `handler`, `source_event`, `exception`, `attempt` (1-based, the call that failed), `delay_seconds`. Suppress with `RetryPolicy(observe="log"|"silent")` |
| `RetryPolicy` | Class | Declarative retry for a handler call: `max_attempts` (counts the initial call), `base_delay`, `max_delay`, `strategy` (`"exponential"`/`"constant"`), `jitter` (full jitter, default `True`), `on=` (subset of `raises=`), `respect_retry_after`, `observe`. Distinct from `langgraph.types.RetryPolicy`, which re-runs a whole node |
...
| `assert_all_baselined_cover` | Function | `assert_all_baselined_cover(serde, baseline_path)` — weakest coverage gate: set-membership over `revivable_identities()`; raises `MigrationCoverageError` if an identity is neither live nor rename-covered. Namespace-walk-scoped. See [coverage gates](event-migrations.md#coverage-gates) |
