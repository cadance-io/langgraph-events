...
| `HandlerRaised` | Event | Emitted when a handler raises a `raises=`-declared exception; carries `handler`, `source_event`, `exception`. The exception keeps its traceback — this is the terminal failure, the one you debug from. With a `retry=` policy it fires only after the budget is spent |
| `HandlerRetried` | Event | Emitted before each `retry=` backoff wait; carries `handler`, `source_event`, `exception`, `attempt` (1-based, the call that failed), `delay_seconds`. The exception instance is live (matchers and field injection work) but its traceback is detached, so a breadcrumb never pins the failed attempt's frame — see [Retries](control-flow.md#retries). Suppress with `RetryPolicy(observe="log"\|"silent")` |
