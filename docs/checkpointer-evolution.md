...
| **Rename / relocate an Event class** | Old checkpoints fail revival under the default serde — author a migration; the [coverage gates](event-migrations.md#coverage-gates) catch an uncovered rename in CI before deploy. A renamed `Interrupted` subclass stays resumable under `@migrate_from` — unlike removing it, which needs `abandon()` first (see above). |
