...
- Renaming a handler with active interrupted threads is safe **if** you declare `@on(previously=...)` (or pin `@on(name=...)` up front); the handler coverage gate flags an undeclared rename in CI.
