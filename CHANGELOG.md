...
### Added
- **agui**: `langgraph_events.agui.drop_reducers("a", "b")` — ergonomic deny-list builder for the common "hide a few internal reducers" case: `AGUIAdapter(..., include_reducers=drop_reducers("debug_count"))`. Resolved at adapter construction against the graph's reducer set into a concrete `list[str]` allow-list, so the runtime path is exact (no wasted compute, no opaque callables on the hot path). Framework-internal channels (`events`, `_cursor`, `_pending`, `_round`) and dedicated AG-UI keys (`messages`) are stripped *before* the user's allow-list runs, so devs never have to remember them. The existing `bool` and `list[str]` forms are unchanged; the same value applies symmetrically to outbound `StateSnapshotEvent` and inbound `RunAgentInput.state` echo (defense-in-depth against stale-client key injection). Malformed `include_reducers` values raise `TypeError` at construction.
